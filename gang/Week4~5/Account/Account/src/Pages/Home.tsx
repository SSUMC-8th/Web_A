...
function Home() {
  ...
  const { data:lps, isLoading, isError, isFetching, hasNextPage , fetchNextPage  } = useGetInfiniteLpList({limit: 20, search:"",order});  
...
<div>
    ...
    <div ref={ref} className="grid sm:grid-cols-3 md:grid-cols-5 gap-2 px-2">  
      {isFetching && Array.from({ length: 20 }).map((_, idx) => <LpBoardSkeleton key={idx} />)}
    </div>  
  </div>
  );
}
