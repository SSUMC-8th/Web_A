...
function useThrottle<T>(value: T, delay: number = 500): T {
  // 1. 상태 변수 : throttledVale : 최종적으로 쓰로틀링이 적용된 값 저장
  // 초기값을 전달받은 value
  const [throttledValue, setThrottledValue] = useState<T>(value);

  // 2. Ref lastExcecuted : 마지막으로 실행된 시간을 기록하는 변수
  // useRef를 사용하면 컴포넌트가 리렌더링 되어도 값이 유지되고, 변경되어도 리렌더링을 트리거하지 않아요.
  const lastExecuted = useRef<number>(Date.now());

  // 3. useEffect: value, delay가 변경될 때 아래 로직 실행.
  useEffect(() => {
    // 현재 시각과 laseExecuted.current에 저장된 마지막 시간 + delay를 비교합니다.
    // 충분한 시간이 지나면 바로 업데이트 합니다.
