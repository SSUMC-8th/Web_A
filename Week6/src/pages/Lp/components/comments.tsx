...
import { useCreateComments } from '../../../features/comments/hooks/useCreateComments';
...
function Comments({ lpId }: { lpId: number }) {
  ...
  const {
    ...
    hasNextPage,
    isFetchingNextPage,
  } = useInfiniteQuery<ResponseGetCommentsDto, Error>({
  ...
  const comments: CommentItem[] = data?.pages.flatMap((p) => p.data.data) ?? [];
